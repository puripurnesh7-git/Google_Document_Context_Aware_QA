...
def identify_use_cases(state: AgentState) -> dict:
    ...
    # CHANGE: Updated prompt to request a structured bulleted list for clarity and consistency.
    prompt = f"""
    Analyze the provided data catalogue and the list of competitor use cases.
    Based *only* on the data available in the catalogue, identify which of the competitor use cases are immediately viable.

    For each viable use case, provide a brief justification explaining which data points from the catalogue make it possible.

    Present your findings as a bulleted list. Do not include use cases that are not possible with the current data.

    **Data Catalogue:**
    {state['data_catalogue_info']}

    **Competitor Use Cases:**
    {json.dumps(state['competitor_use_cases'], indent=2)}
    """
...
def analyze_data_gaps(state: AgentState) -> dict:
    ...
    # CHANGE: Updated prompt to request a structured markdown table to prevent malformed output.
    prompt = f"""
    Analyze the competitor use cases and compare them against the available data in the data catalogue.
    Identify which data points are MISSING to implement each competitor use case.

    Present your findings in a clean, well-formatted markdown table with two columns: "Use Case" and "Missing Data Points".
    - The "Use Case" column should contain the name of the competitor use case.
    - The "Missing Data Points" column should list the specific data points that are required but not found in the data catalogue.
    - If a use case is fully supported by the data catalogue, you can either omit it or state "None".

    Do not add any extra characters, introductory text, or summaries outside of the markdown table itself.
...
def formatted_run_agent(inputs: dict):
    """
            ...
            for node_name, output in event.items():
                ...
                if node_name == "ingest":
                    ...
                    if final_state:
                        ...
                        # CHANGE: Added section to display the data gap analysis in the final report.
                        yield "\n\n### Data Gap Analysis\n"
                        yield final_state.get("missing_data_sources", "_No data gap analysis was performed or results are unavailable._\n")
